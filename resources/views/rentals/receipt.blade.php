@extends('layouts.app')
@php($r = $rental ?? $model)
@section('title', 'Rental Receipt - ' . ($r->claim_code ?? 'Rental Slip'))
...
<div class="row justify-content-center py-3">
    <div class="col-lg-7">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 d-print-none">
            <a href="{{ route('rentals.index') }}" class="text-decoration-none small fw-bold text-success d-inline-flex align-items-center gap-1">
        ...
        <div class="receipt-card">
            ...
            <div class="text-center pb-3 border-bottom mb-4">
                ...
                <div class="badge bg-success-subtle text-success border border-success-subtle px-3 py-1.5 fw-bold" style="font-size: 12px;">
                    OFFICIAL EQUIPMENT RENTAL SLIP
                </div>
            </div>

            <!-- Claim Code Highlight -->
            <div class="p-3 rounded-3 text-center mb-4" style="background-color: #f0fdf4; border: 1.5px dashed #86efac;">
                <div class="text-muted small fw-bold text-uppercase" style="letter-spacing: 0.5px;">Official Rental Claim Code</div>
                <div class="display-6 fw-bold font-monospace text-success my-1">{{ $r->claim_code ?? 'APPROVED' }}</div>
                <div class="text-muted small" style="font-size: 11.5px;">Present this claim code or printed slip at the Barangay Hall to claim items.</div>
            </div>
            ...
            <table class="table table-borderless mb-4" style="font-size: 13.5px;">
                <tbody>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2" style="vertical-align: top;">Rented Items:</td>
                        <td class="fw-bold text-dark text-end py-2">
                            @if ($r->items && count($r->items) > 0)
                                @foreach ($r->items as $line)
                                    <div>{{ $line->quantity }}× {{ $line->equipment->name ?? 'Equipment' }}</div>
                                @endforeach
                            @else
                                <div>Barangay Equipment</div>
                            @endif
                        </td>
                    </tr>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Resident Name:</td>
                        <td class="fw-semibold text-dark text-end py-2">{{ $r->user->name }}</td>
                    </tr>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Contact Number:</td>
                        <td class="text-dark text-end py-2">{{ $r->user->contact_no ?? '—' }}</td>
                    </tr>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Rental Schedule:</td>
                        <td class="fw-semibold text-dark text-end py-2">
                            @if ($r->start_date->eq($r->end_date))
                                {{ $r->start_date->format('F d, Y') }}
                            @else
                    ...
                    <tr class="border-bottom">
                        ...
                        <td class="text-dark text-end py-2">{{ $r->purpose ?? 'Community Activity' }}</td>
                    </tr>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Payment Status:</td>
                        <td class="text-end py-2">
                            <span class="badge {{ $r->payment_status === 'paid' ? 'bg-success' : 'bg-warning text-dark' }} px-2.5 py-1">
                                {{ strtoupper($r->payment_status ?? 'UNPAID') }}
                            </span>
                            <span class="text-muted small">({{ ucfirst($r->payment_method ?? 'Cash') }})</span>
                        </td>
                    </tr>
                    <tr>
                        <td class="text-dark fw-bold fs-6 ps-0 pt-3">Total Amount:</td>
                        <td class="fw-bold fs-5 text-success text-end pt-3">
                            ₱{{ number_format($r->amount ?? $r->amount_due ?? 5.00, 2) }}
                        </td>
                    </tr>
                </tbody>
            </table>
