@extends('layouts.app')
@section('title', 'Booking Receipt - ' . ($booking->claim_code ?? 'Reservation'))
...
<div class="row justify-content-center py-3">
    <div class="col-lg-7">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 d-print-none">
            <a href="{{ route('bookings.index') }}" class="text-decoration-none small fw-bold text-success d-inline-flex align-items-center gap-1">
        ...
        <div class="receipt-card">
            ...
            <div class="text-center pb-3 border-bottom mb-4">
                ...
                <div class="badge bg-success-subtle text-success border border-success-subtle px-3 py-1.5 fw-bold" style="font-size: 12px;">
                    OFFICIAL FACILITY RESERVATION SLIP
                </div>
            </div>

            <!-- Claim Code Highlight -->
            <div class="p-3 rounded-3 text-center mb-4" style="background-color: #f0fdf4; border: 1.5px dashed #86efac;">
                <div class="text-muted small fw-bold text-uppercase" style="letter-spacing: 0.5px;">Official Reservation Claim Code</div>
                <div class="display-6 fw-bold font-monospace text-success my-1">{{ $booking->claim_code ?? 'APPROVED' }}</div>
            ...
            <table class="table table-borderless mb-4" style="font-size: 13.5px;">
                <tbody>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Reserved Facility:</td>
                        <td class="fw-bold text-dark text-end py-2">{{ $booking->facility->name ?? 'Barangay Facility' }}</td>
                    </tr>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Resident Name:</td>
                        <td class="fw-semibold text-dark text-end py-2">{{ $booking->user->name }}</td>
                    </tr>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Contact Number:</td>
                        <td class="text-dark text-end py-2">{{ $booking->user->contact_no ?? '—' }}</td>
                    </tr>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Reserved Dates:</td>
                        <td class="fw-semibold text-dark text-end py-2">
                            @if ($booking->start_date->eq($booking->end_date))
                                {{ $booking->start_date->format('F d, Y') }}
                            @else
                    ...
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Time Schedule:</td>
                        <td class="fw-semibold text-dark text-end py-2">
                    ...
                    <tr class="border-bottom">
                        ...
                        <td class="text-dark text-end py-2">{{ $booking->purpose }}</td>
                    </tr>
                    <tr class="border-bottom">
                        <td class="text-muted ps-0 py-2">Payment Status:</td>
                        <td class="text-end py-2">
                            <span class="badge {{ $booking->payment_status === 'paid' ? 'bg-success' : 'bg-warning text-dark' }} px-2.5 py-1">
                                {{ strtoupper($booking->payment_status) }}
                            </span>
                            <span class="text-muted small">({{ ucfirst($booking->payment_method) }})</span>
                        </td>
                    </tr>
                    <tr>
                        <td class="text-dark fw-bold fs-6 ps-0 pt-3">Total Amount:</td>
                        <td class="fw-bold fs-5 text-success text-end pt-3">
                            ₱{{ number_format($booking->amount_due ?? $booking->amount ?? ($booking->facility->fee ?? 100), 2) }}
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="pt-3 border-top text-center text-muted small" style="font-size: 11px;">
                ...
                <div>Official Facebook Page: Barangay San Jose, Talibon, Bohol &bull; Official Email: blgusanjosetalibon1910@gmail.com</div>
            </div>
        </div>
